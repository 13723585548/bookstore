...
a:hover {
  ...
  <body>
	...
	<div class="divBody">
		...
		<div class="divContent">
			<div style="margin: 20px;">
				...
				<br /> <br /> <br /> <br /> 
				<c:choose>
					<c:when test="${btn eq 'logout' }">
						<span style="margin-left: 50px;"><a
						target="_top" href="<c:url value='/logout.action' />">退出</a></span> 
					</c:when>
					<c:when test="${btn eq 'login' }">
						<span style="margin-left: 50px;"><a
						target="_top" href="<c:url value='/jsp/user/login.jsp' />">登录</a></span> 
					</c:when>
				</c:choose>
				<span
	...
	<div>
		...
		<div>
			<div>
				<img src="<c:url value='${img }'/>" /> <span>${msg }</span>
				<span><a target="_top" href="<c:url value='/jsp/user/login.jsp'/>">登录</a></span> 
				<span><a target="_top" href="<c:url value='/index.jsp'/>">主页</a></span>
				<span><a target="_top" href="<c:url value='/logout.action' />">退出</a></span>
			</div>
		</div>
	</div> --%>
</body>
