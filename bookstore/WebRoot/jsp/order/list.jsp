		...
		<div class="divMain">
			...
			<table align="center" border="0" width="100%" cellpadding="0"
				...
				<c:forEach items="${pageBean.records }" var="order">
					...
					<tr class="tt">
						<td width="320px">订单号：<a href="<c:url value='/findOrderById.action?orderId=${order.id }' />">${order.id }</a></td>
					...
					<tr style="padding-top: 10px; padding-bottom: 10px;">
						<td colspan="2">
							<c:forEach items="${order.orderItemList }" var="orderItem">
								<a class="link2" href="<c:url value='/findBookById.action?bookId=${orderItem.bookId }' />">
						...
						<td>
							<a href="<c:url value='/findOrderById.action?orderId=${order.id }' />">查看</a><br />
							<c:if test="${order.status eq 1 }">
								<a href="<c:url value='/paymentPrepare.action?orderId=${order.id }' />">支付</a>
								<br />
								<a href="<c:url value='/findOrderById.action?orderId=${order.id }&btn=cancel' />">取消</a>
								<br />
							</c:if> <c:if test="${order.status eq 3 }">
								<a href="<c:url value='/findOrderById.action?orderId=${order.id }&btn=confirm' />">确认收货</a>
								<br />
							</c:if>
						</td>
					</tr>
