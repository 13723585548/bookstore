...
<html>
  <head>
	...
	<link rel="stylesheet" type="text/css" href="<c:url value='/jsp/backstage/admin/css/book/list.css' />">
	...
	<script type="text/javascript" src="<c:url value='/jsp/backstage/admin/js/book/list.js' />"></script>
  </head>
  
  <body>
  		<form action="<c:url value='/admin/findBookByCombinationAdmin.action' />" id="search" target="body">
	  		<select id="child" onchange="loadBookList();" name="value">
	  			<c:forEach items="${parents }" var="parent">
		  			<optgroup label="${parent.name }">
		  				<c:forEach items="${parent.children }" var="child">
		  					<option value="${child.id }" 
		  						<c:if test="${child.id eq value}">selected="selected"</c:if>>
		  						${child.name }
		  					</option>
		  				</c:forEach>
		  			</optgroup>
	   		...
	   		<a href="<c:url value='/jsp/backstage/admin/book/seniorSearch.jsp'/>" style="font-size: 10pt; color: #404040;" target="body">高级搜索</a>
	   	</form>
	   	<br/><br/>
    	<ul>
		<c:forEach items="${pageBean.records }" var="book">
			<li>
				<div class="inner">
					<a class="pic" href="<c:url value='/admin/loadBook.action?bookId=${book.id }' />">
					...
					<p>
						<a id="bookname" title="${book.name }"
							href="<c:url value='/admin/loadBook.action?bookId=${book.id }' />">${book.name }</a>
					</p>
					<%-- url标签会自动对参数进行url编码 --%>
					<c:url value="/admin/findBookByConditionAdmin.action" var="authorUrl">
		    			<c:param name="condition" value="author" />			
						<c:param name="value" value="${book.author }"/>
					</c:url>
					<c:url value="/admin/findBookByConditionAdmin.action" var="pressUrl">
